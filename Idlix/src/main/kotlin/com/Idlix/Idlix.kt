// 🌩️ IDLIX MAIN API - Cloudstream Plugin  
// --------------------------------------------------  
// 📦 Package utama
package com.idlix
...
// 🎬 Kelas utama untuk meng-handle API dari situs Idlix
class Idlix : MainAPI() {

    // 🌐 URL dan pengaturan dasar plugin
    ...
    // 🏠 Halaman utama plugin (kategori konten)
    ...
    // 🧭 Ambil base URL dari sebuah link
    ...
    // 📄 Ambil konten dari halaman utama
    override suspend fun getMainPage(
        ...
        val req = if (nonPaged) app.get(request.data)
        else app.get("${url.first()}$page/?${url.lastOrNull()}")
    ...
    // 🧩 Atur link yang mengarah ke halaman film/series sebenarnya
    ...
    // 🔍 Konversi elemen HTML menjadi hasil pencarian
    ...
    // 📥 Ambil detail film / series (poster, rating, aktor, dll)
    override suspend fun load(url: String): LoadResponse {
        ...
        val title = document.selectFirst("div.data > h1")?.text()
            ?.replace(Regex("\\(\\d{4}\\)"), "")?.trim().toString()
        ...
        val year = Regex(",\\s?(\\d+)").find(document.select("span.date").text().trim())
        ...
        val description = if (tvType == TvType.Movie)
            document.select("div.wp-content > p").text().trim()
        else
        ...
        val recommendations = document.select("#single_relacionados article").map {
            ...
            newMovieSearchResponse(
                recName, recHref,
                if (recHref.contains("/movie/")) TvType.Movie else TvType.TvSeries, false
            ) {
                this.posterUrl = recPosterUrl
            }
        }

        // 📺 Jika TV Series, ambil episode list
        return if (tvType == TvType.TvSeries) {
            val episodes = document.select("ul.episodios > li").map {
                val href = it.select("a").attr("href")
                val name = fixTitle(it.select("div.episodiotitle > a").text().trim())
                val image = it.select("div.imagen > img").attr("src")
                val episode = it.select("div.numerando").text().replace(" ", "")
                    .split("-").last().toIntOrNull()
                val season = it.select("div.numerando").text().replace(" ", "")
    ...
    // 🔗 Load link streaming dan subtitle
    override suspend fun loadLinks(
        ...
        val scriptRegex =
            """window\.idlixNonce=['"]([a-f0-9]+)['"].*?window\.idlixTime=(\d+).*?""".toRegex(RegexOption.DOT_MATCHES_ALL)
        ...
        document.select("ul#playeroptionsul > li").map {
            ...
            val json = app.post(
                ...
                data = mapOf(
                    ...
                    "_n" to idlixNonce, "_p" to id, "_t" to idlixTime
                ),
                referer = data,
    ...
    // 🧹 Bersihkan karakter berlebih dari hasil decrypt
    private fun String.fixBloat(): String {
        return this.replace("\"", "").replace("\\", "")
    }

    // 🧩 Data model untuk response JSON
