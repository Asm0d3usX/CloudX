// 🎬 LayarKacaProvider - Cloudstream API
// --------------------------------------------------
// 📦 Package utama
...
class LayarKacaProvider : MainAPI() {
    ...
    // 🏠 Halaman utama & kategori populer
    ...
    // 🔗 Mendapatkan link sebenarnya dari halaman
    ...
    // 🔍 Ubah elemen HTML menjadi SearchResponse
    private fun Element.toSearchResult(): SearchResponse? {
        ...
        return if (type == TvType.TvSeries) {
            val episode = selectFirst("span.episode strong")?.text()?.filter { it.isDigit() }?.toIntOrNull()
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        val res = app.get("$mainUrl/search.php?s=$query").text
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val year = Regex("\\d, (\\d+)").find(document.select("div.movie-info h1").text().trim())
    ...
    // 🔗 Load semua link video dan subtitle
    ...
    // 🌐 Ambil URL dasar dari hasil redirect
    ...
    // 🖼️ Ambil atribut gambar yang sesuai
