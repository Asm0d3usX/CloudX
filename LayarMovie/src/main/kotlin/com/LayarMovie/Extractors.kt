...
/**
 * ============================================
 *   🎬 LayarMovie - Custom Extractors Package
 *   Berisi beberapa extractor untuk berbagai
 *   domain streaming mirror.
 * ============================================
 */

/* ------------------------
   🧩 Subclass dari Dingtezuni
   (semua menggunakan struktur mirip Earnvids)
   ------------------------ */
...
/* ------------------------
   🎥 Extractor utama - Dingtezuni
   Digunakan oleh beberapa domain mirroring di atas
   ------------------------ */
...
open class Dingtezuni : ExtractorApi() {
    ...
    override suspend fun getUrl(
        ...
        // Ambil script yang berisi link M3U8 (baik packed atau langsung)
        val script = if (!getPacked(response.text).isNullOrEmpty()) {
            var result = getAndUnpack(response.text)
            if (result.contains("var links")) result.substringAfter("var links") else result
        } else {
            response.document.selectFirst("script:containsData(sources:)")?.data()
        } ?: return

        // m3u8 urls bisa diawali dengan file:, hls2:, atau hls4:
    ...
    /** Mengonversi berbagai bentuk URL ke embed /v/ */
...
/* ------------------------
   📦 Gofile Extractor
   Mendapatkan konten video dari Gofile API
   ------------------------ */
...
open class Gofile : ExtractorApi() {
    ...
    override suspend fun getUrl(
        ...
        // Ambil semua file video dari API Gofile
    ...
    /** Ekstraksi resolusi dari nama file */
...
/* ------------------------
   🌐 Lain-lain (berbasis StreamWish & VidStack)
   ------------------------ */
